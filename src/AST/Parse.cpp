...
bool Parsing::parseBlock(ParseHelper &p, StmtBlock *&tree, bool with_brace)
{
	...
	Map<StringRef, StringRef> attributes;
	...
	while(p.isValid() && (!with_brace || !p.accept(lex::RBRACE) || only_one_stmt)) {
		...
		if(stmt) {
			if(stmt->isReturn() && !inserted_defers) {
...
bool Parsing::parseType(ParseHelper &p, StmtType *&type)
{
	...
	if(p.acceptn(lex::BAND)) stmtmask |= (uint16_t)StmtMask::REF;
	if(p.acceptn(lex::CONST)) stmtmask |= (uint16_t)StmtMask::CONST;
		       ...
		       const Occurs &oval)
{
	...
	while(p.accept(lex::STATIC, lex::VOLATILE) || p.accept(lex::GLOBAL, lex::COMPTIME)) {
		if(p.acceptn(lex::COMPTIME)) stmtmask |= (uint16_t)StmtMask::COMPTIME;
		if(p.acceptn(lex::STATIC)) stmtmask |= (uint16_t)StmtMask::STATIC;
		if(p.acceptn(lex::VOLATILE)) stmtmask |= (uint16_t)StmtMask::VOLATILE;
		if(p.acceptn(lex::GLOBAL)) stmtmask |= (uint16_t)StmtMask::GLOBAL;
	}

	bool comptime = stmtmask & (uint16_t)StmtMask::COMPTIME;
	bool stati    = stmtmask & (uint16_t)StmtMask::STATIC;
	bool volatil  = stmtmask & (uint16_t)StmtMask::VOLATILE;
	bool global   = stmtmask & (uint16_t)StmtMask::GLOBAL;
...
bool Parsing::parseFnSig(ParseHelper &p, Stmt *&fsig)
{
	...
	if(!p.acceptn(lex::FN)) {
		err::out(p.peek(), "expected 'fn' here, found: ", p.peek().tokCStr());
		return false;
	}
...
bool Parsing::parseExtern(ParseHelper &p, Stmt *&ext)
{
	...
	if(!p.acceptn(lex::EXTERN)) {
		err::out(p.peek(), "expected extern keyword here, found: ", p.peek().tokCStr());
		return false;
	}
	...
	if(!p.accept(lex::IDEN)) {
		err::out(p.peek(),
			 "expected identifier or string here, found: ", p.peek().tokCStr());
		return false;
	}
	name = p.peek();
	p.next();
	...
	if(struct_kw) name.setDataStr(toString("struct ", name.getDataStr()));
...
bool Parsing::parseEnum(ParseHelper &p, Stmt *&ed)
{
	...
	if(!p.acceptn(lex::ENUM)) {
		err::out(p.peek(), "expected 'enum' keyword here, found: ", p.peek().tokCStr());
		return false;
	}
	if(p.acceptn(lex::COL) && !parseType(p, tagty)) {
		err::out(p.peek(), "expected tag type for enum, found: ", p.peek().tokCStr());
		return false;
	}
	...
	if(!p.acceptn(lex::RBRACE)) {
		err::out(p.peek(),
			 "expected right braces for end of enum list, found: ", p.peek().tokCStr());
		return false;
	}
	if(enumvars.empty()) {
...
bool Parsing::parseStruct(ParseHelper &p, Stmt *&sd, bool allowed_templs)
{
	...
	lex::Lexeme &start = p.peek();
	bool is_union	   = p.accept(lex::UNION);
	bool is_decl	   = false;

	if(!p.acceptn(lex::STRUCT, lex::UNION)) {
		err::out(p.peek(),
			 "expected 'struct' or 'union' keyword here, found: ", p.peek().tokCStr());
		return false;
	}
	...
	while(p.accept(lex::IDEN, lex::COMPTIME, lex::ATTRS)) {
		if(p.accept(lex::ATTRS) && !parseAttributes(p, attributes)) return false;
		...
		if(!attributes.empty()) {
			field->setAttributes(std::move(attributes));
			attributes.clear();
		}
	...
	sd = StmtStruct::create(ctx, start.getLoc(), fields, templates, is_union, is_decl);
	return true;
}
...
bool Parsing::parseReturn(ParseHelper &p, Stmt *&ret)
{
	...
	if(!p.acceptn(lex::RETURN)) {
		err::out(p.peek(), "expected 'return' here, found: ", p.peek().tokCStr());
		return false;
	}
...
bool Parsing::parseContinue(ParseHelper &p, Stmt *&cont)
{
	...
	if(!p.acceptn(lex::CONTINUE)) {
		err::out(p.peek(), "expected 'continue' here, found: ", p.peek().tokCStr());
		return false;
	}
...
bool Parsing::parseBreak(ParseHelper &p, Stmt *&brk)
{
	...
	if(!p.acceptn(lex::BREAK)) {
		err::out(p.peek(), "expected 'break' here, found: ", p.peek().tokCStr());
		return false;
	}
...
bool Parsing::parseDefer(ParseHelper &p, Stmt *&defer)
{
	...
	if(!p.acceptn(lex::DEFER)) {
		err::out(p.peek(), "expected 'defer' here, found: ", p.peek().tokCStr());
		return false;
	}

	if(!parseBlock(p, (StmtBlock *&)val, true)) {
		err::out(p.peek(), "failed to parse defer block");
		return false;
	}

	deferstack.addStmt(val);
	return true;
}
bool Parsing::parseAttributes(ParseHelper &p, Map<StringRef, StringRef> &attrs)
{
	...
	for(size_t i = 0; i < data.size(); ++i) {
		...
		if(c == ',') {
			...
			attrs[key] = valstart == -1 ? "" : StringRef(&data[valstart], vallen);
			keystart = valstart = -1;
			keylen = vallen = 0;
			continue;
		}
