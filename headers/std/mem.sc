...
let alloc = fn(comptime T: type, count: u64): *T {
...
let cpy = fn(dest: any, src: const any, count: u64): any {
	return @as(@typeOf(dest), _memcpy(@as(@ptr(void), dest), @as(@ptr(void), src), count));
};
let set = fn(dest: any, ch: i32, count: u64): any {
	return @as(@typeOf(dest), _memset(@as(@ptr(void), dest), ch, count));
};
let cmp = fn(lhs: const any, rhs: const any, count: u64): i32 {
